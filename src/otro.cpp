...
void Dibujar(const char* rutaImagen, double posx, double posy, double posz, double tam)
{
	...
	glBegin(GL_POLYGON);
	glColor3f(1, 1, 1);
	glTexCoord2d(1, 1);    glVertex3f(posx, posy, posz);  // Abajo izquierda
	glTexCoord2d(0, 1);    glVertex3f(posx + tam, posy, posz);   // Abajo derecha
	glTexCoord2d(0, 0);    glVertex3f(posx + tam, posy + tam, posz);    // Arriba derecha
	glTexCoord2d(1, 0);    glVertex3f(posx, posy + tam,posz);   // Arriba izquierda
